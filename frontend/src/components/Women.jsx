...
function Women() {
    ...
    const fetchWomensProducts = async () => {
      try {
        ...
        if (data.success) {
          ...
          setWomenClothing(data.data.slice(0, 5))
        }
      } catch (error) {
  ...
  return (
    <section className="py-12 px-4 md:px-16 lg:px-24 xl:px-32 bg-white">
      <h1 className="text-3xl font-bold text-slate-900 text-center mb-2">Women's Clothing</h1>
      <p className="text-slate-600 mb-10 text-center">Discover elegance and style in every piece</p>
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          <p className="text-gray-600">No women's products available yet.</p>
        </div>
      ) : (
